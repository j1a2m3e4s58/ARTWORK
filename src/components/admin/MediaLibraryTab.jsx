        ...
        {items.filter(item => item.purpose === 'content-library').map(item => (
          <article key={item.id} className="overflow-hidden border border-brass/10 bg-carbon">
            ...
            <div className="space-y-3 p-4">
              ...
              <p className={`text-[10px] uppercase tracking-wider ${item.scanStatus === 'clean' ? 'text-green-300' : item.scanStatus === 'client-encrypted' ? 'text-blue-300' : 'text-amber-300'}`}>Safety scan: {item.scanStatus || 'not configured'} · Processing: {item.processingStatus || 'legacy'}</p>
