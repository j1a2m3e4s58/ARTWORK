...
export default function RecycleBinTab() {
  ...
  const purgeItems = async targets => {
    ...
    const wording = targets.length === items.length ? 'everything in the recycle bin' : `${targets.length} selected item${targets.length === 1 ? '' : 's'}`;
    if (!window.confirm(`Permanently delete ${wording}? This cannot be undone.`)) return;
    setPurging(true);
    setError('');
    try {
    ...
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canPurge && items.length > 0 && <div className="flex flex-wrap gap-2">
          ...
          <button disabled={!selectedItems.length || purging} onClick={() => purgeItems(selectedItems)} className="min-h-10 border border-red-400/30 px-3 text-xs text-red-300 disabled:cursor-not-allowed disabled:opacity-40">Delete selected ({selectedItems.length})</button>
          <button disabled={purging} onClick={() => purgeItems(items)} className="min-h-10 bg-red-500/15 px-3 text-xs text-red-200 hover:bg-red-500/25 disabled:opacity-40">Delete all</button>
        </div>}
      </div>
          ...
          {items.map(item => {
            ...
            return <div key={itemKey(item)} className="flex flex-col gap-3 border border-brass/10 bg-carbon p-4 sm:flex-row sm:items-center">
              ...
              <div className="flex gap-2"><button onClick={() => restore(item)} className="flex min-h-10 flex-1 items-center justify-center gap-2 border border-brass/25 px-3 text-xs text-brass sm:flex-none"><RotateCcw size={14} /> Restore</button>
                {canPurge && <button disabled={purging} onClick={() => purgeItems([item])} className="min-h-10 border border-red-400/25 px-3 text-xs text-red-300 disabled:opacity-40">Delete</button>}
              </div>
            </div>;
          })}
