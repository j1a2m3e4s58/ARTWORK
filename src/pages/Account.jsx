              ...
              <div className="border border-brass/10 bg-carbon p-5">
                ...
                <div className="mt-4 max-h-72 space-y-2 overflow-y-auto">
                  {data.notifications.map(item => (
                    <article key={item.id} className={`overflow-hidden border ${item.read ? 'border-ivory/5 bg-obsidian' : 'border-brass/25 bg-brass/5'}`}>
                      {item.imageUrl && <img src={item.imageUrl} alt="" loading="lazy" className="max-h-36 w-full object-cover" />}
                      <div className="p-3">
                        <div className="flex justify-between gap-3"><strong className="text-sm">{item.title}</strong><time className="shrink-0 text-[10px] text-ivory/30">{new Date(item.created_date).toLocaleDateString()}</time></div>
                        <p className="mt-1 text-xs text-ivory/50">{item.message}</p>
                        {item.attachmentUrl && item.attachmentType === 'document' && <a href={item.attachmentUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex min-h-8 items-center rounded-full border border-brass/25 px-3 text-[10px] font-semibold uppercase tracking-wider text-brass">Open document</a>}
                      </div>
                    </article>
                  ))}
                  {!data.notifications.length && <p className="py-5 text-center text-sm text-ivory/35">No notifications in this view.</p>}
                </div>
