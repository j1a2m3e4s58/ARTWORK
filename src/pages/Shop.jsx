...
import { Download, FileText, Heart, List, Search, ShoppingBag, X } from 'lucide-react';
...
import ArtFinderRequest from '@/components/shop/ArtFinderRequest';
...
export default function Shop() {
  ...
  return (
    <PageTransition>
      <main className="min-h-screen bg-obsidian pb-28 pt-28">
        ...
        <header className="mx-auto mb-12 max-w-7xl px-5 sm:px-6 lg:px-12">
          <div className="flex flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="flex max-w-full flex-wrap gap-2 xl:flex-nowrap xl:justify-end xl:gap-1.5"><button onClick={() => setFinderOpen(true)} className="flex min-h-12 shrink-0 items-center gap-2 bg-brass px-3 text-xs text-obsidian 2xl:px-4 2xl:text-sm"><Search size={18}/> Studio Art Finder</button><Link to={user?.role === 'partner' ? '/partner-portal' : '/partner-with-us'} className="flex min-h-12 shrink-0 items-center gap-2 border border-brass/30 px-3 text-xs text-brass transition-colors hover:border-brass/60 2xl:px-4 2xl:text-sm">{user?.role === 'partner' ? 'Partner portal' : 'Sell with us'}</Link><button onClick={() => setCatalogueOpen(true)} className="flex min-h-12 shrink-0 items-center gap-2 border border-brass/30 px-3 text-xs text-brass transition-colors hover:border-brass/60 2xl:px-4 2xl:text-sm"><List size={18} /> Art catalogue</button><button onClick={() => setGuidesOpen(true)} className="flex min-h-12 shrink-0 items-center gap-2 border border-brass/30 px-3 text-xs text-brass transition-colors hover:border-brass/60 2xl:px-4 2xl:text-sm"><FileText size={18} /> Price guides</button><button onClick={() => setCartOpen(true)} className="relative flex min-h-12 w-fit shrink-0 items-center gap-2 border border-brass/30 px-3 text-xs text-ivory/70 transition-colors hover:border-brass/60 hover:text-brass 2xl:px-4 2xl:text-sm">
      ...
      <ArtFinderRequest open={finderOpen} onClose={() => setFinderOpen(false)} />
