...
export default function OrderTracking() {
  ...
  const submit = async event => {
    ...
    try { setOrder(await studioClient.orders.track(form)); }
    catch (requestError) { setError(requestError.message || 'We could not find that order.'); }
  ...
  return <PageTransition><main className="min-h-screen bg-obsidian px-5 pb-24 pt-28 sm:px-6">
    <section className="mx-auto max-w-3xl">
      ...
      <h1 className="mt-3 font-display text-5xl text-ivory sm:text-6xl">Track an order</h1>
      <p className="mt-4 max-w-xl text-sm leading-7 text-ivory/55">Enter the order tracking code and the email used at checkout. Your address and payment details remain private.</p>
      <form onSubmit={submit} className="mt-8 grid gap-3 border border-brass/15 bg-carbon p-5 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
        ...
        <button disabled={loading} className="flex min-h-12 items-center justify-center gap-2 bg-brass px-5 text-xs font-semibold uppercase tracking-wider text-obsidian disabled:opacity-50"><Search size={15} />{loading ? 'Checking…' : 'Track'}</button>
      </form>
      {error && <p role="alert" className="mt-4 border border-red-400/20 bg-red-400/5 p-3 text-sm text-red-200">{error}</p>}
      {order && <section className="mt-8 border border-brass/15 bg-carbon p-5 sm:p-7">
